...
export default function MspAssetsContracts() {
  ...
  function openContract(contract = null) {
  ...
  async function saveContract(event) {
    ...
    const result = editingContract ? await supabase.from('service_contracts').update(payload).eq('id', editingContract) : await supabase.from('service_contracts').insert(payload)
    if (result.error) setError(result.error.message)
    else { setMessage(editingContract ? 'Service contract updated.' : 'Service contract created.'); setContractOpen(false); await load() }
    setSaving('')
  }
