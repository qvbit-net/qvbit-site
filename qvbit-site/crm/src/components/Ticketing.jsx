...
export default function Ticketing() {
  ...
  async function createTicket(event) {
    ...
    else { setShowNew(false); setDraft({ subject: '', description: '', requester_name: '', requester_email: '', requester_phone: '', issue_type: '', priority: 'normal' }); await loadTickets(); if (data) loadDetail(data) }
    setBusy(false)
  }
  ...
  async function updateStatus(status) {
    ...
    if (updateError) setError(updateError.message); else { setSelected(data); await loadTickets() }
  }
  ...
  async function addNote(event) {
    ...
    if (noteError) setError(noteError.message); else { setNote(''); loadDetail(selected) }
  }
  ...
  async function sendReply(event) {
    ...
    try {
      ...
      setReply({ subject: `Re: ${selected.subject}`, body: '' }); setAttachments([]); loadDetail(selected); updateStatus('waiting_on_customer')
  ...
  return <div className="page-stack">
    ...
    <div className="two-column-layout" style={{ gridTemplateColumns: selected ? 'minmax(260px, 0.8fr) minmax(0, 1.5fr)' : '1fr' }}>
      <section className="panel-card"><div className="toolbar"><input placeholder="Search tickets…" value={search} onChange={(e) => setSearch(e.target.value)} /><select value={filter} onChange={(e) => setFilter(e.target.value)}><option value="all">All statuses</option>{STATUSES.map((s) => <option key={s} value={s}>{label(s)}</option>)}</select></div>{loading ? <p className="muted">Loading tickets…</p> : visible.length === 0 ? <p className="muted">No tickets found.</p> : <div className="list-stack">{visible.map((ticket) => <button key={ticket.id} className={`list-row ${selected?.id === ticket.id ? 'active' : ''}`} onClick={() => loadDetail(ticket)}><span><strong>#{ticket.ticket_number} · {ticket.subject}</strong><small>{ticket.requester_name || ticket.requester_email || 'Internal ticket'} · {label(ticket.status)}</small></span><span className={`status-badge ${ticket.priority}`}>{label(ticket.priority)}</span></button>)}</div>}</section>
